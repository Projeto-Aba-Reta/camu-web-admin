...
Área Vendas do dashboard: torna o item "Vendas/Marketplace" da sidebar clicável com `/vendas/pedidos` como página padrão, organiza a área em abas (Pedidos, Funil, Resultado e Configurações) exibidas conforme o perfil do usuário, e define o guard de acesso às rotas `/vendas/**` e às Server Actions, além da ocultação das ações de escrita para quem só tem permissão de leitura.
