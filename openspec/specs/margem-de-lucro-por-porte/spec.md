# margem-de-lucro-por-porte

## Purpose

Margem de lucro própria de cada faixa de porte (P/M/G), configurada separadamente para B2C e B2B com modo de aplicação somar/substituir, versionada por vigência junto com a faixa, e a regra de resolução da margem efetiva que o motor de cálculo usa para projetar preço.
...
#### Scenario: Precificação define a margem do porte G
...
O sistema SHALL permitir escrita da margem de lucro por faixa de porte apenas a usuários `owner`/`socio` ou com a role `precificacao`, e leitura a `owner`/`socio` ou role `precificacao`/`producao` — mesma regra de acesso já aplicada às faixas de porte e às taxas por canal.
...
- **THEN** o sistema rejeita a escrita, pois margem de lucro é responsabilidade de Precificação
