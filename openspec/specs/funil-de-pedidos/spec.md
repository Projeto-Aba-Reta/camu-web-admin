...
Andamento de cada pedido de venda pelas etapas do funil: um pedido ocupa exatamente uma etapa ativa, move-se livremente para frente ou para trás, exige impressora do parque nas etapas assim marcadas e deixa um histórico somente-adição de cada passagem. Cobre também o quadro kanban agrupado por etapa, o recorte que tira pedidos finalizados antigos da visão e as regras de acesso à movimentação.
...
### Requirement: Movimentação para a etapa adjacente
O sistema SHALL oferecer, no cartão de cada pedido do quadro, dois atalhos de movimentação — um para a etapa ativa imediatamente anterior e outro para a imediatamente seguinte, na ordem cadastrada do funil. Cada atalho SHALL permitir registrar uma nota opcional na passagem e SHALL pedir a impressora quando a etapa de destino a exigir. O atalho correspondente SHALL ser omitido quando não houver etapa adjacente naquele sentido.

#### Scenario: Avançar uma etapa
- **WHEN** um usuário aciona o atalho de avançar em um pedido que está em "Aguardando embalagem"
- **THEN** o sistema move o pedido para "Embalando", a etapa ativa seguinte, e registra a passagem no histórico

#### Scenario: Voltar uma etapa com nota
- **WHEN** um usuário aciona o atalho de voltar em um pedido e escreve a nota "peça saiu com defeito"
- **THEN** o sistema move o pedido para a etapa ativa anterior e grava a nota no evento de histórico

#### Scenario: Pedido na primeira etapa do funil
- **WHEN** um pedido está na primeira etapa ativa do funil
- **THEN** o cartão não oferece o atalho de voltar, apenas o de avançar

#### Scenario: Etapa adjacente que exige impressora
- **WHEN** um usuário aciona o atalho de avançar e a etapa seguinte exige impressora
- **THEN** o sistema pede a impressora antes de concluir a movimentação
