...
Fila de itens de produção vinculados ao catálogo, com ciclo de vida `na_fila` → `imprimindo` → `concluido` (ou cancelamento em `na_fila`/`imprimindo` → `cancelado`). Atribui uma impressora do parque no início da impressão, gera baixa automática de estoque (peça pronta + insumo) na conclusão e envia notificação via Slack.
